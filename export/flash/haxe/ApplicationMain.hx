...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("ProblemDemo1", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("ProblemDemo1", null, 800, 600);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "45";
		app.meta["company"] = "BASTEC";
		app.meta["file"] = "ProblemDemo1";
		app.meta["name"] = "ProblemDemo1";
		app.meta["packageName"] = "org.bastec.ProblemDemo1";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("ProblemDemo1")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess ProblemDemo1.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"ProblemDemo1\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"ProblemDemo1\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends ProblemDemo1 {}
#else
class DocumentClass
{
